...
export function GenerateAzureCliInit(model: CodeModelCli) : string[] {
    ...
    output.push("");
    output.push("from azext_" + model.GetCliCommand() + "._help import helps  # pylint: disable=unused-import");
    output.push("");
    output.push("");
    ...
    output.push("        from azext_" + model.GetCliCommand() + "._client_factory import cf_" + model.GetCliCommand() + "");
    output.push("        " + model.GetCliCommand() + "_custom = CliCommandType(");
    output.push("            operations_tmpl='azext_" + model.GetCliCommand() + ".custom#{}',");
    output.push("            client_factory=cf_" + model.GetCliCommand() + ")");
    output.push("        super( " + model.ServiceNameX + "CommandsLoader, self).__init__(cli_ctx=cli_ctx,");
    output.push("                                                       custom_command_type=" + model.GetCliCommand() + "_custom)");
    output.push("");
    output.push("    def load_command_table(self, args):");
    output.push("        from azext_" + model.GetCliCommand() + ".commands import load_command_table");
    output.push("        load_command_table(self, args)");
    output.push("        return self.command_table");
    output.push("");
    output.push("    def load_arguments(self, command):");
    output.push("        from azext_" + model.GetCliCommand() + "._params import load_arguments");
